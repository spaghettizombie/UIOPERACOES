# Manual Operacional — Plataforma Táctica

Documento técnico-operacional destinado a operadores, analistas e oficiais responsáveis pelo planeamento e execução de operações. Estabelece as funcionalidades disponíveis, os procedimentos de utilização e os fluxos operacionais recomendados.
...
A escolha é mantida em sessão e pode ser alterada em qualquer momento através do indicador no canto inferior direito da barra de estado. A alteração de perfil não compromete os dados operacionais já registados.
...
| Acção | Procedimento |
|---|---|
| Deslocação do mapa | Arrastamento com botão esquerdo do rato |
| Aproximação/afastamento | Roda do rato ou controlos `+`/`−` |
...
| Operação nocturna ou leitura em condições de baixa luminosidade | CartoDB Dark |
...
A plataforma disponibiliza trinta tipos de marcadores agrupados por função operacional. Os marcadores correspondentes a meios institucionais portugueses utilizam fundos cromáticos de identidade institucional, com texto e contorno brancos para garantir contraste e legibilidade.

### 3.1 Métodos de criação

- **Barra de ferramentas:** activação do instrumento de marcação e selecção da posição no mapa.
...
Apresentadas pela ordem hierárquica operacional (do escalão de comando para o apoio especializado):

| Sigla | Designação | Identidade visual |
|---|---|---|
| PC | Posto de Comando | Fundo verde escuro · contorno e texto brancos |
| PCM | Posto de Comando Móvel | Fundo verde escuro · contorno e texto brancos |
| GIO | UI — GIOE | Fundo vermelho · contorno e texto brancos |
| GIP | UI — GIOP | Fundo preto · contorno e texto brancos |
| PAT | GNR — Patrulha | Fundo verde escuro · contorno e texto brancos |
| PSP | PSP — Patrulha | Fundo azul escuro · contorno e texto brancos |
| BV | BV — Bombeiros | Fundo vermelho · contorno e texto brancos · símbolo de chama |
| INM | INM — INEM | Fundo azul · contorno e texto brancos · cruz médica |
| K9 | K9 — Cinotecnia | Fundo preto · contorno e texto brancos |
| EOD | EOD — Engenhos / Inativação | Fundo preto · contorno e texto brancos |
| SNP | Sniper | Fundo vermelho · contorno e texto brancos |
| STK | Stack (pré-entrada) | Cor padrão |
| BRE | Breach point | Cor padrão |
| NEG | Negociador | Fundo verde escuro · contorno e texto brancos |
| DRN | Drone / UAV | Fundo verde escuro · contorno e texto brancos |
| FRP | Força amiga (genérica) | Cor padrão |

A diferenciação cromática entre meios com identidade institucional consolidada (PC, PCM, GIO, GIP, PAT, PSP, BV, INM, K9, EOD, SNP, NEG, DRN) e meios tácticos genéricos (STK, BRE, FRP) permite identificação imediata em vista táctica.
...
A reposicionação é efectuada por arrastamento.
...
Sequência de cliques para definição dos vértices, finalizando com duplo-clique ou tecla Enter. Apresenta a distância total e por segmento.

### 4.2 Área poligonal
Definição dos vértices com cliques sucessivos. O duplo-clique fecha o polígono. O resultado é apresentado em metros quadrados ou quilómetros quadrados, conforme magnitude.

### 4.3 Raio circular
Dois cliques: centro e ponto de raio. Apresenta o raio em metros.

### 4.4 Azimute geodésico
Dois cliques: origem e destino. Apresenta o ângulo (0–360°) e a distância.
...
Sequência de waypoints com duplo-clique para finalizar. Apresenta a distância acumulada.

### 4.8 Zona de exclusão
Dois cliques: centro e raio. Visualização distinta com tracejado vermelho.

### 4.9 Barreira / Corte de estrada
Dois cliques nas extremidades da barreira. Gera um rectângulo escuro perpendicular ao vector definido, com etiqueta indicativa do comprimento.

### 4.10 Quadrícula militar (MGRS)
Sobreposição de grelha conforme padrão militar. Activável também através da tecla `Q`.

### 4.11 Limpeza
Acção de limpeza global de medições, reversível através do histórico (Ctrl+Z).
...
Dois controlos no canto superior direito do mapa permitem a utilização de geolocalização.
...
A configuração da chave é efectuada através do menu contextual (botão direito) sobre o controlo de itinerários na barra de ferramentas.
...
As credenciais são mantidas localmente.
...
| Forças Amigas | PC, PCM, GIO, GIP, PAT, PSP, BV, INM, K9, EOD, SNP, STK, BRE, NEG, DRN, FRP |
...
5. Marcação das forças a empenhar conforme o respectivo escalão (PC/PCM, GIO, GIP, PAT, PSP, BV, INM, K9, EOD, SNP, NEG, DRN);
